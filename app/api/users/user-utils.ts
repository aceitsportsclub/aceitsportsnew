...
export function createStandaloneSupabaseClient() {
...
export function createAdminSupabaseClient() {
  ...
  if (!serviceKey) throw new Error('SUPABASE_SERVICE_ROLE_KEY is required for admin user operations.');
...
export async function getAuthUserAndProfile(
  ...
  // This covers browser requests that rely on cookie-based sessions instead of Bearer headers
  if (!token) {
    ...
    if (cookieMatch?.[1]) {
      try {
        ...
        // Cookie value may be JSON array [access_token, ...] or plain JWT string
...
export function assertCanManageUsers(profile: { role: string; clubs: string[] }) {
  if (profile.role === 'OWNER') return true;
  if (profile.clubs && profile.clubs.length > 0) return true;
  throw new Error('FORBIDDEN');
}

export function toSourceUser(
  profileRow: Record<string, unknown>,
  roleName: string = 'STUDENT',
  clubSlugs: string[] = [],
  permissions: string[] = ['profile.view', 'profile.edit', 'clubs.join']
) {
  const id = String(profileRow.id || '');
  const createdAt = (profileRow.created_at as string) || new Date().toISOString();
  const updatedAt = (profileRow.updated_at as string) || createdAt;
  ...
  return {
    ...
    rtuRollNo: (profileRow.rtu_roll_no as string) || '',
    branch: (profileRow.branch as string) || '',
    year: (profileRow.year as string) || '',
    position: (profileRow.position as string) || '',
    jerseyNo: (profileRow.jersey_no as string) || '',
    height: (profileRow.height as string) || '',
    mobile: (profileRow.mobile as string) || '',
    sport: (profileRow.sport as string) || '',
    photo: (profileRow.photo as string) || null,
    bio: (profileRow.bio as string) || '',
    active: profileRow.active !== false,
    role: roleName,
    clubs: clubSlugs,
    clubId: clubSlugs[0] || '',
    permissions: roleName === 'OWNER' ? ['*'] : permissions,
    createdAt,
